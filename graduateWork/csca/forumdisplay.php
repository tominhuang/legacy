...
require_once MYBB_ROOT."inc/settings_csca.php";//tomin
require_once MYBB_ROOT."inc/functions_csca.php";//tomin
...
{
	$newPost= "<h3><a href=newthread.php?fid={$fid}&icon=claim>發表想法</a></h3>";
	//eval("\$newthread = \"".$templates->get("forumdisplay_newthread")."\";");		
	eval("\$newthread = \"".$newPost."\";");		
...
if($fpermissions['canviewthreads'] != 0)
{
	...
	//tomin
	$query = $db->query("
		SELECT t.*, p.displaystyle AS threadprefix, {$ratingadd}{$select_rating_user}t.username AS threadusername, u.username, po.message
		FROM ".TABLE_PREFIX."threads t
		LEFT JOIN ".TABLE_PREFIX."users u ON (u.uid = t.uid){$select_voting}
		LEFT JOIN ".TABLE_PREFIX."threadprefixes p ON (p.pid = t.prefix)
		LEFT JOIN ".TABLE_PREFIX."posts po ON (po.tid = t.tid) and (po.icon = 7) 
