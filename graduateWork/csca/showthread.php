...
require_once MYBB_ROOT."inc/functions_csca.php";
...
if($mybb->input['action'] == "thread")
{
	...
	if($mybb->input['mode'] == 'threaded')
	{
		...
		$query = $db->query("
            SELECT p.username, p.uid, p.pid, p.replyto, p.subject, p.dateline, p.message, p.icon, i.path
            FROM ".TABLE_PREFIX."posts p
			JOIN ".TABLE_PREFIX."icons i ON i.iid=p.icon 
            WHERE p.tid='$tid'
            $visible
            ORDER BY p.dateline
        ");
	...
	{						
		...
		{		
			/*	
				實驗組
				最外層-發表想法
				查看想法-提出證據、提出反駁、評論(with sentence opener)
				查看證據-說明理由、評論
				查看理由-評論
				查看反駁-評論
				查看評論-提出證據、提出反駁、說明理由、發表想法、評論
			...
			*/			
			...
			$data = "<a href=newreply.php?tid={$tid}&pid={$pid}&icon=data>提出證據</a>";
			$rebuttal = "<a href=newreply.php?tid={$tid}&pid={$pid}&icon=rebuttal>提出反駁</a>";
			$challenge = "<a href=newreply.php?tid={$tid}&pid={$pid}&icon=challenge>挑戰</a>";
			$check = "<a href=newreply.php?tid={$tid}&pid={$pid}&icon=check>檢驗</a>";
			$warrant = "<a href=newreply.php?tid={$tid}&pid={$pid}&icon=warrant>說明理由</a>";
			$comment = "<a href=newreply.php?tid={$tid}&pid={$pid}&icon=comment>發表評論</a>";

			$viewClaim = $data. "、" . $rebuttal . "、". $comment;
			$viewData = $warrant. "、" . $comment;
			$viewWarrant = $comment;
			$viewRebuttal = $comment;
			$viewChallenge = $comment;
			$viewCheck = $comment;
			$viewComment = $data. "、" . $rebuttal . "、". $warrant. "、" . $comment;			
												
						
			$replyItems = $viewClaim;
			
			if( $icon == $settings['icon_data'] ) $replyItems = $viewData ;
			else if( $icon == $settings['icon_rebuttal'] ) $replyItems = $viewRebuttal ;
			else if( $icon == $settings['icon_warrant'] ) $replyItems = $viewWarrant ;
			else if( $icon == $settings['icon_challenge'] ) $replyItems = $viewChallenge ;
			else if( $icon == $settings['icon_check'] ) $replyItems = $viewCheck ;
			else if( $icon == $settings['icon_comment'] ) $replyItems = $viewComment ;
			
			$replyPost= "<span class=elementsButton>{$replyItems}</span>";
			eval("\$newreply = \"".$replyPost."\";");			
...
var message = j(".message1");
/*
var subjectMaxLength = 10000;

j.each(message,function(){
	if(j(this).html().length > subjectMaxLength ){			
		j(this).html( j(this).text().substr(0,subjectMaxLength) + " .." ); 
	}
});
*/
...
	查看評論-提出證據、提出反駁、說明理由、發表想法、評論
	*/			
	...
	var data = "<a href=newreply.php?tid=" + tid  + "&pid=" + pid + "&icon=data>提出證據</a>";
	var rebuttal = "<a href=newreply.php?tid=" + tid  + "&pid=" + pid + "&icon=rebuttal>提出反駁</a>";
	var warrant = "<a href=newreply.php?tid=" + tid  + "&pid=" + pid + "&icon=warrant>說明理由</a>";
	var comment = "<a href=newreply.php?tid=" + tid  + "&pid=" + pid + "&icon=comment>發表評論</a>";

	var viewClaim = data + "、" + rebuttal + "、" + comment;
	var viewData = warrant + "、" + comment;
	var viewWarrant = comment;
	var viewRebuttal = comment;
	var viewComment= comment;			
			
	var replyItems = viewClaim;
	...
	replyItems = "  " + replyItems + "<br><br>";	
	j(this).parents(".threadboxline").find(".replyitems").html(replyItems);
	
	if(uid==<?=$mybb->user['uid']?>){//is Author
		var editlink = "<a href=editpost.php?pid=" + pid + ">編輯</a>、<a href=editpost.php?pid=" + pid + "  onclick=\"Thread.deletePost(" + pid + "); return false;\">刪除</a>";
		j(this).parents(".threadboxline").find(".edit").html(editlink);
	}				
});
j(".message2").click(function(){
	j(this).hide();
	j(this).parent().find(".message1").show();
	j(this).parents(".threadboxline").find(".replyitems").html("");
	j(this).parents(".threadboxline").find(".edit").hide();
	return false;
});

var inDevilForum = <?=isInDevilForum()?>;
if(inDevilForum){
	j("#scaffolding").show();
}
</script>