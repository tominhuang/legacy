...
if($mybb->input['action'] == "do_newreply" && $mybb->request_method == "post")
{
	...
	$plugins->run_hooks("newreply__start");
...
if($mybb->input['action'] == "newreply" || $mybb->input['action'] == "editdraft")
{
		...
		if(count($quoted_posts) > 0)
		{
			...
			while($quoted_post = $db->fetch_array($query))
			{
				...
				if($quoted_post['tid'] == $tid || $load_all == 1)
				{
					...
					//tomin
					if($quote) $message .= parse_quoted_message($quoted_post);
	...
	if($mybb->settings['threadreview'] != 0)
	{
		...
		//tomin
		$query = $db->query("
			SELECT p.*, u.username AS userusername, i.path						
			FROM ".TABLE_PREFIX."posts p
		...
		eval("\$threadreview2 = \"".$templates->get("newreply_threadreview")."\";");
	}
...
var element="<?=$_GET['icon']?>";


j("#subject").html("無法取得");
if(element=="data"){
	j("input[type='radio'][value='1']").click();
	j("#subject").html("[證據]");
	j("input[name=subject]").val("[證據]");
	j("#message").val("\n\n\n\n\n資料來源：");
	j("#sentenceOpenData").show();
}else if(element=="rebuttal"){
	j("input[type='radio'][value='2']").click();
	j("#subject").html("[反駁]");
	j("input[name=subject]").val("[反駁]");
	j("#sentenceOpenRebuttal").show();
}else if(element=="warrant"){
	j("input[type='radio'][value='9']").click();
	j("#subject").html("[理由]");
	j("input[name=subject]").val("[理由]");
	j("#sentenceOpenWarrant").show();
}else if(element=="challenge"){
	j("input[type='radio'][value='10']").click();
	j("#subject").html("[挑戰]");
	j("input[name=subject]").val("[挑戰]");
	j("#trsentence").hide();
}else if(element=="check"){
	j("input[type='radio'][value='11']").click();
	j("#subject").html("[檢驗]");
	j("input[name=subject]").val("[檢驗]");
	j("#trsentence").hide();
}else if(element=="comment"){
	j("input[type='radio'][value='13']").click();
	j("#subject").html("[評論]");
	j("input[name=subject]").val("[評論]");
	j("#trsentence").hide();
	
	var inDevilForum = <?=isInDevilForum()?>;
	if(inDevilForum){
		j(".trScaffolding").show();
	}
	
}else{
	j("#trsentence").hide();
}
j(".sentence_opener").click(function(){
	var message = j("textarea[name='message_new']").val();	
	var sentence_opener = j(this).html();	

	j("textarea[name='message_new']").val( sentence_opener + message );
	j("textarea[name='message_new']").focus();
	
	return false;
});
j(".moves").click(function(){
	var rel = j(this).attr("rel");	
	j("#"+rel).toggle();
});
j("form[name=input] .tcat .smalltext").html("<?=$topic?>");
j("input[name=submit]").click(function(){
	if(element=="data"){
		alert("證據發表中……接下來你可以說明理由或發表評論");
	}else if(element=="rebuttal"){
		alert("反駁發表中……接下來你可以發表評論");
	}else if(element=="warrant"){
		alert("理由發表中……接下來你可以發表評論");
	}else if(element=="comment"){
		alert("評論發表中……接下來你可以提出證據、提出反駁或發表評論");		
	}
});
var inDevilForum = <?=isInDevilForum()?>;
if(inDevilForum){
