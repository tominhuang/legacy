...
function jumpArticle(list, start, move){
	...
	$.ajax({
	  ...
	  success: function(data){
		...
		if(start==="" && move === "prev") current = $(data).find("dl tr td a:last").parents("dd");
		else if(start==="" && move === "next") current = $(data).find("dl tr td a:first").parents("dd");
		...
		while(true){
			if(move === "prev"){
				...
				url = current.find("td:eq(3) a").attr("href");
			}else{
				if(start!==""){
		...
		if(!url){
			...
			if(move === "prev"){
				...
				url = jumpArticle(pageURL, "", "prev");
			}else if(move === "next"){
				pageURL = $(data).find("#prodlist a:contains('下一頁')").prop('href');
				url = jumpArticle(pageURL, "", "next");			
			}		
		}
